...
// 1. Thêm hàm getSelectedSSID() (ở đâu đó trong file .cpp, ví dụ gần đầu)
String WifiManager::getSelectedSSID()
{
    // Kiểm tra xem có mạng nào được chọn hợp lệ không
    if (selectedIndex >= 0 && selectedIndex < apCount)
    {
        return aps[selectedIndex].ssid; // Trả về SSID của mạng tại chỉ mục đang chọn
    }
    return ""; // Trả về chuỗi rỗng nếu không hợp lệ
}

// 2. Sửa hàm drawInputPasswordScreen()
void WifiManager::drawInputPasswordScreen()
{
    String ssidToDisplay = getSelectedSSID(); // LẤY SSID Ở ĐÂY
